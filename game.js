...
function showRewardCelebration(type, data={}){
  const old=document.getElementById('reward-celebration');
  if(old) old.remove();
  const overlay=document.createElement('div');
  overlay.id='reward-celebration';
  overlay.setAttribute('aria-hidden','true');
  const isQuest=type==='quest';
  const title=isQuest ? 'QUÊTE TERMINÉE !' : 'SUCCÈS DÉBLOQUÉ !';
  const name=data.name ? escapeHTML(data.name) : (isQuest ? 'Objectif atteint' : 'Nouveau succès');
  const reward=Number(data.reward)||0;
  overlay.innerHTML=`
    <div class="reward-celebration-flash" aria-hidden="true"></div>
    <div class="reward-celebration-rays" aria-hidden="true"></div>
    <div class="reward-celebration-card">
      <div class="reward-celebration-icon">${isQuest?'🎯':(data.icon||'🏆')}</div>
      <div class="reward-celebration-kicker">${title}</div>
      <strong>${name}</strong>
      ${reward?`<span>+${reward} 🪙</span>`:''}
    </div>`;

  // Vraies pièces CSS plutôt que des emojis : rendu identique sur mobile/PC.
  ...
  for(let i=0;i<count;i++){
    ...
    const coin=i%3!==0;
    if(coin) piece.classList.add('reward-coin');
    else piece.classList.add('reward-spark');
    const left=(Math.random()*108-4).toFixed(2);
    const drift=(Math.random()*260-130).toFixed(0);
    const rotate=(Math.random()*1100-550).toFixed(0);
    const delay=(Math.random()*0.18).toFixed(3);
    const duration=(1.35+Math.random()*0.7).toFixed(2);
    const size=(coin ? 10+Math.random()*9 : 5+Math.random()*7).toFixed(0);
    piece.style.left=left+'vw';
    piece.style.width=size+'px'; piece.style.height=size+'px';
    piece.style.setProperty('--fall-x',drift+'px');
    ...
    // Web Animations API : ne dépend pas du @media prefers-reduced-motion.
  ...
  requestAnimationFrame(()=>{
    ...
    const pieces=overlay.querySelectorAll('.reward-piece');
    pieces.forEach(piece=>{
      const duration=parseFloat(piece.style.getPropertyValue('--fall-duration'))*1000;
      const delay=parseFloat(piece.style.getPropertyValue('--fall-delay'))*1000;
      const drift=piece.style.getPropertyValue('--fall-x');
      const rot=piece.style.getPropertyValue('--fall-rotate');
      if(piece.animate){
        piece.animate([
          {transform:'translate3d(0,-5vh,0) rotate(0deg) scale(.55)',opacity:0},
          {transform:'translate3d(0,7vh,0) rotate(35deg) scale(1)',opacity:1,offset:.08},
          {transform:`translate3d(${drift},118vh,0) rotate(${rot}) scale(1.05)`,opacity:.95}
        ],{duration,delay,easing:'cubic-bezier(.18,.72,.24,1)',fill:'forwards'});
      } else {
        piece.classList.add('reward-piece-fallback');
      }
    });
  });
  clearTimeout(window._rewardCelebrationTimer);
  window._rewardCelebrationTimer=setTimeout(()=>{
...
window.claimAchievement=function(id,btnEl){
  ...
  saveAchievementData(d);
  addCoins(a.reward);
  showRewardCelebration('achievement', a);
  updateMenuDisplays();
...
function updateDailyQuest(event, amount=1){
  ...
  data.quests.forEach(q=>{
    ...
    if(q.event===event){
      ...
      const wasComplete=before>=q.target;
      q.progress=Math.min(q.target,before+amount);
      if(q.progress!==before) changed=true;
      if(!wasComplete && q.progress>=q.target) newlyCompleted.push(q);
    }
  });
  if(event==='combo'){
    ...
    if(human && (human.wallsPlacedThisGame||0)>=3 && (human.cardsUsedThisGame||0)>=1){
      data.quests.forEach(q=>{
        if(q.event==='combo'&&!q.claimed){
          const wasComplete=q.progress>=q.target;
          q.progress=1; changed=true;
          if(!wasComplete) newlyCompleted.push(q);
        }
      });
    }
  }
...
const RANKS = [
  { name: 'Légende ultime', icon: '👑', min: 10000 },
  { name: 'Immortel', icon: '✨', min: 7500 },
  { name: 'Mythique', icon: '🔱', min: 5000 },
  { name: 'Légende', icon: '🌟', min: 4000 },
  { name: 'Champion', icon: '🏆', min: 3000 },
  { name: 'Élite', icon: '⚔️', min: 2000 },
  { name: 'Grand maître', icon: '👑', min: 1500 },
...
// Route des Trophées étendue : les anciens paliers sont conservés,
// puis la progression continue jusqu'à 10 000 trophées.
const TROPHY_ROAD = [
  ...
  { req: 1350,  rank: null,             icon: '',   reward: 700 },
  { req: 1500,  rank: 'Grand maître',   icon: '👑', reward: 1000 },
  { req: 1750,  rank: null,             icon: '',   reward: 900 },
  { req: 2000,  rank: 'Élite',          icon: '⚔️', reward: 1500 },
  { req: 2250,  rank: null,             icon: '',   reward: 1000 },
  { req: 2500,  rank: null,             icon: '',   reward: 1800 },
  { req: 2750,  rank: null,             icon: '',   reward: 1200 },
  { req: 3000,  rank: 'Champion',       icon: '🏆', reward: 2200 },
  { req: 3500,  rank: null,             icon: '',   reward: 1500 },
  { req: 4000,  rank: 'Légende',        icon: '🌟', reward: 3000 },
  { req: 4500,  rank: null,             icon: '',   reward: 1800 },
  { req: 5000,  rank: 'Mythique',       icon: '🔱', reward: 4000 },
  { req: 5500,  rank: null,             icon: '',   reward: 2200 },
  { req: 6000,  rank: null,             icon: '',   reward: 5000 },
  { req: 6500,  rank: null,             icon: '',   reward: 2500 },
  { req: 7000,  rank: null,             icon: '',   reward: 6000 },
  { req: 7500,  rank: 'Immortel',       icon: '✨', reward: 5000 },
  { req: 8000,  rank: null,             icon: '',   reward: 7000 },
  { req: 9000,  rank: null,             icon: '',   reward: 8000 },
  { req: 10000, rank: 'Légende ultime', icon: '👑', reward: 12000 }
];
...
function openCardPack() {
  // Sécurité V14 : une célébration ou un ancien état de pack ne doit jamais bloquer l'ouverture.
  const celebration = document.getElementById('reward-celebration');
  if (celebration) celebration.remove();
  if (packOpenPhase !== 'idle') {
    // Si aucun overlay de pack actif n'existe, on réinitialise proprement l'état.
    const activePack = document.getElementById('pack-opening-overlay');
    if (!activePack || activePack.classList.contains('hidden')) {
      packOpenPhase = 'idle';
      pkoClearTimers();
    } else return;
  }
...
// V14 : exposition explicite pour les boutons inline et les navigateurs mobiles.
...
let gameLoadingActive = false;

function startGame() {
  if (gameLoadingActive) return;
  gameLoadingActive = true;
  tutorialActive = false;
  document.getElementById('tutorial-coach')?.remove();
  uiLocked = true;
  showScreen('game');

  const loader = document.getElementById('game-loading-screen');
  const fill = document.getElementById('game-loading-fill');
  const subtitle = document.getElementById('game-loading-subtitle');
  if (loader) {
    loader.classList.remove('hidden');
    loader.setAttribute('aria-hidden','false');
    if (fill) fill.style.width = '0%';
    if (subtitle) subtitle.textContent = 'Mise en place du plateau...';
  }

  const steps = [
    [18, 'Mise en place du plateau...'],
    [48, 'Préparation des joueurs...'],
    [76, 'Chargement des éléments...'],
    [100, 'Bonne chance !']
  ];
  const loadingDuration = 1150;
...
function openMenuPanel(type) {
  ...
  if (type === 'modes') {
    ...
    // Sur mobile, on se replace directement sur le prochain palier.
    // Cela évite de devoir faire défiler toute la route pour retrouver sa progression.
    const trophyTarget = body.querySelector('.tr-card.target');
    if (trophyTarget) {
      requestAnimationFrame(() => {
        trophyTarget.scrollIntoView({ block: 'center', behavior: 'smooth' });
      });
    }
...
function setupEventListeners() {
  ...
  document.getElementById('menu-panel').addEventListener('click', (event) => {
    const packBtn = event.target.closest && event.target.closest('.pack-open-btn');
    if (packBtn) {
